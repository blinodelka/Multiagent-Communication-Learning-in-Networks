...
def play_many_games_semisupervised(num_agents, num_episodes, inner_speech, learning_rate, punishment_weight, punishment_talk, agents_memory, replay, beta_1, p_peek, num_choose_act, degree, verbose=False):
...
for superv in supervision:
    for deg in degrees:
        for j in range(samples):
            cond_list.append([superv, deg])
            
supervision_rate = cond_list[trial_index][0]
deg = cond_list[trial_index][1]
d = play_many_games_semisupervised(10, 120000, 0, 0.0001, 4, 0, 10, 10, 0.3, supervision_rate, 4, deg)
