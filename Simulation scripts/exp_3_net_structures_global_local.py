...
def play_many_games_semisupervised(num_agents, num_episodes, inner_speech, learning_rate, punishment_weight, punishment_talk, agents_memory, replay, beta_1, p_peek, num_choose_act, global_local, verbose=False):
    ...
    G = nx.newman_watts_strogatz_graph(num_agents, 2, gl) 
...
for superv in supervision:
    for gl in gls:
        for j in range(samples):
            cond_list.append([superv, gl])
            
supervision_rate = cond_list[trial_index][0]
global_local = cond_list[trial_index][1]
d = play_many_games_semisupervised(10, 120000, 0, 0.0001, 4, 0, 10, 10, 0.3, supervision_rate, 4, global_local)
